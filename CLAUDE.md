...
```text
Release ONDEWO Survey Typescript Client ${ONDEWO_SURVEY_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO Survey Typescript Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO Survey Typescript Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/survey-client-typescript`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/survey-client-typescript versions --json
```
